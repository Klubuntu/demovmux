  ...
  return (
    <header className="fixed top-0 left-0 lg:left-60 right-0 h-14 bg-white border-b border-gray-200 flex items-center justify-between px-3 lg:px-6 z-30">
      ...
      <div className="flex items-center gap-2 md:gap-4">
        {/* mobile status dot – only when nav is hidden */}
        {svc && (
          <span
            className={`md:hidden w-2.5 h-2.5 rounded-full flex-shrink-0 ${statusConfig[svc.status].dot}`}
            title={statusConfig[svc.status].label}
          />
        )}
