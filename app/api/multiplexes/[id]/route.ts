...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const mux = await dbGet('SELECT * FROM multiplexes WHERE id = ?', [id]);
  if (!mux) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  const channels = await dbAll('SELECT * FROM channels WHERE mux_id = ? ORDER BY lcn', [id]);
  const sfn = await dbAll('SELECT * FROM sfn_nodes WHERE mux_id = ? ORDER BY name', [id]);
  const psi = await dbAll('SELECT * FROM psi_si_tables WHERE mux_id = ? ORDER BY table_type', [id]);
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await dbRun(`
    ...
    WHERE id=@id
  `, { mux_type: 'terrestrial', radio_enabled: 0, ...body, id });
  const row = await dbGet('SELECT * FROM multiplexes WHERE id = ?', [id]);
  return NextResponse.json(row);
}

export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  await dbRun('DELETE FROM multiplexes WHERE id = ?', [id]);
