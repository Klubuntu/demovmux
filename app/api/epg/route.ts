...
export async function POST(req: NextRequest) {
  try {
    ...
    `, { channel_id, title, description: description ?? null, start_at, end_at, category: category ?? null, language: language ?? 'pl', episode_num: episode_num ?? null, series_id: series_id ?? null, rating: rating ?? null, poster_url: poster_url ?? null });

    const row = await dbGet('SELECT * FROM epg_programs WHERE id=?', [result.lastInsertRowid]);
