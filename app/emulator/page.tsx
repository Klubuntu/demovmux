...
import {
  ...
  Wifi, Satellite, Radio, Signal,
} from 'lucide-react';

interface Channel {
  id: number; name: string; lcn: number; mux_name: string;
  mux_type: string; video_format: string; channel_type: string; status: string;
}

type ReceptionType = 'dvbt2' | 'dvbs2' | 'dvbc' | 'iptv';

const RECEPTION: Record<ReceptionType, { label: string; color: string; ring: string; bg: string; Icon: React.ElementType }> = {
...
const SHORTCUTS = [
  ...
  { key: '← / →',    desc: 'Głośność -/+' },
  { key: 'Enter',    desc: 'OK / Zamknij menu' },
...
const MENU_ITEMS = ['Lista kanałów', 'Ustawienia obrazu', 'Ustawienia dźwięku', 'Tryb odbioru', 'Informacje o systemie'];
...
export default function EmulatorPage() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setProgress(p => {
        if (p >= 100) {
          clearInterval(t);
          setTimeout(() => { setBooted(true); setPowered(true); }, 500);
          return 100;
        }
        return p + 1.6;
      });
  ...
  useEffect(() => {
    fetch('/api/channels')
      .then(r => r.json())
      .then((d: Channel[]) => setChannels(d.filter(c => c.status === 'active')));
  }, []);
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'Enter':      e.preventDefault(); if (showMenu) { flashOsd(MENU_ITEMS[menuIdx]); setShowMenu(false); } break;
        case 'Escape': case 'Backspace':
          setShowMenu(false); setShowInfo(false); setShowHelp(false); break;
  ...
  }, [chUp, chDown, volUp, volDown, cycleReception, pressNum, powered, showMenu, menuIdx, flashOsd]);
        ...
        <div className="flex flex-col items-center gap-7 text-center px-6">
          ...
          <div className="w-72 space-y-2">
            ...
            <p className="text-gray-600 text-xs font-mono">
              {progress < 30 ? 'Inicjalizacja sprzętu…' :
               progress < 60 ? 'Ładowanie modułów DVB…' :
               progress < 85 ? 'Skanowanie kanałów…' :
               progress < 99 ? 'Uruchamianie GUI…' : 'Gotowy.'}
    ...
    <div className="bg-gray-950 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl min-h-[calc(100vh-7rem)]">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-gray-900 border-b border-gray-800 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <span className="text-white font-bold text-sm shrink-0">vMUX Emulator</span>
          <button
            ...
            title="Kliknij aby zmienić typ odbioru (T)"
          >
            <rec.Icon size={11} />
            {rec.label}
          </button>
          ...
          <div className="w-full max-w-2xl">
            ...
            <div className="bg-gradient-to-b from-gray-750 to-gray-800 rounded-2xl p-3 pb-4 shadow-[0_0_60px_rgba(0,0,0,0.8)] border border-gray-700">
              ...
              <div className="relative bg-black rounded-xl overflow-hidden" style={{ aspectRatio: '16/9' }}>
                ...
                {powered && !switching && (
                  <div className="absolute inset-0">
                    {/* bg gradient per channel */}
                    <div
                      className="absolute inset-0"
                      style={{
                        background: `radial-gradient(ellipse at 30% 40%, hsl(${(chIdx * 47 + 200) % 360}, 30%, 12%) 0%, #0a0a0a 70%)`,
                      }}
                    />
                    ...
                    {ch ? (
                          ...
                          <div className="text-center">
                            ...
                            <div className="flex justify-center items-end gap-0.5 mt-5">
                              {Array.from({ length: 24 }).map((_, i) => (
                                <div
                                  key={i}
                                  className="w-1 rounded-sm bg-blue-500 animate-pulse"
                                  style={{
                                    height: `${8 + Math.abs(Math.sin(i * 0.6) * 18)}px`,
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <p className="text-gray-700 text-sm">Brak kanałów</p>
                      </div>
                    )}
                      ...
                      <div className="absolute top-3 left-3 right-3 flex items-start justify-between pointer-events-none">
                        ...
                        <div className="bg-black/75 backdrop-blur-sm rounded-lg px-3 py-1.5">
                          <div className="flex items-center gap-2.5">
                            <span className={`text-2xl font-black leading-none ${rec.color}`}>
                              {String(ch.lcn).padStart(2, '0')}
                            </span>
                            <div>
                          ...
                          <div className="bg-black/75 backdrop-blur-sm rounded-lg px-2.5 py-1 flex items-center gap-1.5">
                            <div className="flex items-end gap-0.5">
                              {[3, 5, 7, 9].map((h, i) => (
                                <div
                                  key={i}
                                  className={`w-1 rounded-sm ${sig > i * 25 ? rec.color.replace('text', 'bg') : 'bg-gray-700'}`}
                                  style={{ height: `${h}px` }}
                                />
                              ))}
                            </div>
                    ...
                    {osd?.vol && (
                      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-black/85 backdrop-blur-sm rounded-xl px-5 py-3 min-w-52">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-white text-xs w-7 text-right tabular-nums">
                            {muted ? '🔇' : volume}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex items-end">
                        <div className="w-full bg-gray-950/95 p-4 border-t border-gray-700">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <button onClick={() => setShowInfo(false)} className="text-gray-500 hover:text-white transition-colors p-1 rounded">
                              <X size={16} />
                            </button>
                          </div>
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                            {([
                              ['LCN', ch.lcn],
                              ['MUX', ch.mux_name],
                              ['Format', ch.video_format],
                              ['Odbiór', rec.label],
                              ['Sygnał', `${sig}%`],
                              ['Status', 'Aktywny'],
                              ['Typ', ch.channel_type === 'radio' ? '📻 Radio' : '📺 TV'],
                              ['MUX typ', ch.mux_type?.toUpperCase()],
                      ...
                      <div className="absolute inset-0 bg-black/60 flex items-center justify-start p-6">
                        <div className="bg-gray-900/98 rounded-xl border border-gray-700 overflow-hidden w-60 shadow-2xl">
                          ...
                          {MENU_ITEMS.map((item, i) => (
                            <button
                              key={item}
                              onClick={() => { flashOsd(item); setShowMenu(false); setMenuIdx(i); }}
                              className={`w-full text-left px-4 py-3 text-sm flex items-center gap-3 transition-colors border-b border-gray-800/50 last:border-0 ${
                                menuIdx === i
                                  ? `${rec.bg} ${rec.color} font-semibold`
              ...
              <div className="flex items-center justify-between mt-3 px-2">
                <span className="text-gray-700 text-[10px] tracking-widest uppercase">vMUX TV · {rec.label}</span>
                <div className={`w-2 h-2 rounded-full transition-all duration-500 ${
                  powered ? `animate-pulse ${rec.color.replace('text', 'bg')}` : 'bg-red-700'
                }`} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-b from-gray-800 to-gray-900 rounded-3xl p-5 w-48 shadow-[0_8px_40px_rgba(0,0,0,0.7)] border border-gray-700 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className={`w-11 h-11 rounded-full flex items-center justify-center transition-all active:scale-90 shadow-lg ${
                  powered
                    ? 'bg-red-600 hover:bg-red-700 shadow-red-900/60 text-white'
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {['1','2','3','4','5','6','7','8','9','','0',''].map((n, i) => (
                <button
                  ...
                  className={`h-9 rounded-xl text-sm font-mono font-bold transition-colors active:scale-90 ${
                    n ? 'bg-gray-700 hover:bg-gray-600 text-white shadow-sm' : 'pointer-events-none'
                  }`}
                >
                  {n}
                </button>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {([
                ...
                >
                  {icon}
                  <span className="text-[9px] text-gray-400 mt-0.5 font-mono">{label}</span>
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center justify-center">
              <div className="relative w-32 h-32">
                {/* up */}
                <button onClick={chUp} title="CH+ (↑)"
                  className="absolute top-0 left-1/2 -translate-x-1/2 w-11 h-11 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center text-white transition-colors active:scale-90 shadow">
                  <ChevronUp size={18} />
                </button>
                {/* down */}
                <button onClick={chDown} title="CH− (↓)"
                  className="absolute bottom-0 left-1/2 -translate-x-1/2 w-11 h-11 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center text-white transition-colors active:scale-90 shadow">
                  <ChevronDown size={18} />
                </button>
                {/* left */}
                <button onClick={volDown} title="VOL− (←)"
                  className="absolute left-0 top-1/2 -translate-y-1/2 w-11 h-11 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center text-white transition-colors active:scale-90 shadow">
                  <ChevronLeft size={18} />
                </button>
                {/* right */}
                <button onClick={volUp} title="VOL+ (→)"
                  className="absolute right-0 top-1/2 -translate-y-1/2 w-11 h-11 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center text-white transition-colors active:scale-90 shadow">
                  <ChevronRight size={18} />
                </button>
                {/* OK */}
                <button
                  onClick={() => { if (showMenu) { flashOsd(MENU_ITEMS[menuIdx]); setShowMenu(false); } else flashOsd('OK'); }}
            ...
            <div className="grid grid-cols-3 gap-1.5">
              ...
              <button onClick={toggleMute}
                className={`h-9 rounded-xl text-[10px] flex items-center justify-center gap-1 transition-colors active:scale-90 ${
                  muted ? 'bg-red-700 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'
                }`}>
            ...
            <div className="grid grid-cols-4 gap-1">
              {([
                ...
                ['bg-blue-600   hover:bg-blue-500',   () => flashOsd('EPG')],
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-700 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
              ...
              <button onClick={() => setShowHelp(false)}
                className="text-gray-600 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-gray-800">
                <X size={18} />
              </button>
            </div>
            <div className="p-5 space-y-2.5">
              {SHORTCUTS.map(({ key, desc }) => (
                <div key={key} className="flex items-center justify-between gap-4">
                  ...
                  <kbd className="shrink-0 bg-gray-800 text-gray-200 text-xs px-2.5 py-1 rounded-lg border border-gray-700 font-mono">
                    {key}
                  </kbd>
                </div>
              ))}
            ...
            <div className="px-5 py-4 border-t border-gray-800 text-center">
              <p className="text-xs text-gray-600">
                Kliknij typ odbioru (top bar lub pilot) aby zmieniać sygnał · kliknij przyciski pilota lub używaj klawiatury
              </p>
            </div>
          </div>
        </div>
